...
def max_comparable_replan_steps(args):
    return args.max_replan_steps - args.overlap_L - 1


def compute_overlap_score(prev_chunk, new_chunk, prev_r, L, start_j, dims, args):
    prev_chunk = np.asarray(prev_chunk, dtype=np.float32)
    new_chunk = np.asarray(new_chunk, dtype=np.float32)
    dims = np.asarray(dims)

    prev_diff = np.diff(prev_chunk[:, dims], axis=0)
    new_diff = np.diff(new_chunk[:, dims], axis=0)
    ...
    for j in range(start_j, start_j + L):
        ...
        if prev_idx >= len(prev_diff) or new_idx >= len(new_diff):
            return None

        diff = prev_diff[prev_idx] - new_diff[new_idx]
        scores.append(np.sum(diff ** 2))

    return float(np.sum(scores))

def max_comparable_replan_steps(args):
    # 10 actions -> 9 diffs, valid diff indices 0..8.
    # We compare prev_diff[prev_r] with new_diff[0].
    return args.max_replan_steps - 2


def compute_overlap_score(prev_chunk, new_chunk, prev_r, L, start_j, dims, args):
    """
    Compare only the first available diff-overlap:

        prev_diff[prev_r] vs new_diff[0]

    L and start_j are ignored.
    """
    prev_chunk = np.asarray(prev_chunk, dtype=np.float32)
    new_chunk = np.asarray(new_chunk, dtype=np.float32)
    dims = np.asarray(dims)
...
def choose_next_replan_steps(current_r, overlap_score, ema_score, args):
                ...
                try:
                    ...
                    if not action_plan:
                        ...
                        if args.adaptive and prev_chunk is not None and prev_r is not None:
                            max_comparable_r = max_comparable_replan_steps(args)

                            if prev_r > max_comparable_r:
                                overlap_score = None
                                decision = "no_overlap_after_full_horizon"

                                current_r = max_comparable_r

                                logging.info(
                                    f"Adaptive replan: prev_r={prev_r} has no valid diff-overlap. "
                                    f"Resetting next_r={current_r}"
                                )
                        ...
                        current_r = int(np.clip(
                            current_r,
                            args.min_replan_steps,
                            args.max_replan_steps,
                        ))
